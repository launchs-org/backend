...
type IngressRoute struct {
	...
	Name        string             `gorm:"type:varchar(20);not null;default:''"            json:"name"`         // ユーザー設定名（プロジェクト内ユニーク）
	PendingName string             `gorm:"type:varchar(20);default:''"                    json:"pending_name"` // 未適用の名前変更
	Host        string             `gorm:"type:varchar(253);not null"                     json:"host"`         // 払い出し済みドメイン（{name}-{8文字}.{BASE_DOMAIN}）
