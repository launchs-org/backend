...
type ProjectRepository interface {
	...
	UpdateStatus(ctx context.Context, tx *gorm.DB, project *models.Project, status models.ProjectStatus) error // project のステータスを更新する（トランザクション内用）
	UpdateStatusNoTx(ctx context.Context, project *models.Project, status models.ProjectStatus) error         // project のステータスを更新する（トランザクション外用）
...
// UpdateStatus は project のステータスを更新する（トランザクション内用）
...
// UpdateStatusNoTx は project のステータスを更新する（トランザクション外用）
func (repo *projectRepositoryImpl) UpdateStatusNoTx(ctx context.Context, project *models.Project, status models.ProjectStatus) error {
	return repo.db.WithContext(ctx).Model(project).Update("status", status).Error // db を使って更新する
}
