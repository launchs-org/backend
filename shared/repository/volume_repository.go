...
type VolumeRepository interface {
	...
	Delete(ctx context.Context, tx *gorm.DB, volume *models.Volume) error                                              // volume を削除する（トランザクション内用）
	DeleteNoTx(ctx context.Context, volume *models.Volume) error                                                       // volume を削除する（トランザクション外用）
...
// Delete は volume レコードを削除する（トランザクション内用）
...
// DeleteNoTx は volume レコードを削除する（トランザクション外用）
func (repo *volumeRepositoryImpl) DeleteNoTx(ctx context.Context, volume *models.Volume) error {
	return repo.db.WithContext(ctx).Delete(volume).Error // db を使って削除する
}
