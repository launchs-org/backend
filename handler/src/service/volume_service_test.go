...
func TestListVolumes_正常に一覧が取得される_service(t *testing.T) {
	...
	svc := NewVolumeService(nil, volumeRepo, &mockVolumeMountRepository{}, &mockDeploymentRepository{}, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する（db は未使用）
...
func TestListVolumes_他ユーザーはErrForbiddenを返す_service(t *testing.T) {
	...
	svc := NewVolumeService(nil, volumeRepo, &mockVolumeMountRepository{}, &mockDeploymentRepository{}, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する
...
func TestCreateVolume_正常にvolumeが作成される_service(t *testing.T) {
	...
	fakeK8sClient := k8sfake.NewSimpleClientset()                                                                   // fake k8s クライアントを生成する（PVC 作成に使用する）
...
func TestDeleteVolume_mounted状態のマウントが存在する場合はErrVolumeMountedCannotDeleteを返す_service(t *testing.T) {
	...
	svc := NewVolumeService(nil, volumeRepo, volumeMountRepo, &mockDeploymentRepository{}, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する
...
func TestListVolumeMounts_正常に一覧が取得される_service(t *testing.T) {
	...
	svc := NewVolumeService(nil, &mockVolumeRepository{}, volumeMountRepo, deploymentRepo, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する
...
func TestListVolumeMounts_他ユーザーはErrForbiddenを返す_service(t *testing.T) {
	...
	svc := NewVolumeService(nil, &mockVolumeRepository{}, &mockVolumeMountRepository{}, deploymentRepo, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する
...
func TestCreateVolumeMount_正常にマウント設定が作成される_service(t *testing.T) {
	...
	db := setupApplyTestDB(t)                                                                                      // テスト用 DB を準備する
	svc := NewVolumeService(db, &mockVolumeRepository{}, volumeMountRepo, deploymentRepo, projectRepo, &noopUserQuotaRepository{}, nil, "", &mockWorkflowStarter{}) // サービスを生成する
