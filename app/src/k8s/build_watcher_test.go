...
func (mock *mockDeploymentRepoForBuild) UpdatePendingGithubBuildFields(ctx context.Context, deploymentID string, repoURL string, branch string, commitSHA string, directory string) error {
	return nil // テストでは使用しないためデフォルト nil を返す
}
...
func (mock *mockDeploymentRepoForBuild) UpdateDeploymentStatus(ctx context.Context, deploymentID string, status models.DeploymentStatus) error {
	return nil // テストでは使用しない
}
...
func (mock *mockDeploymentRepoForBuild) CreateWithTx(ctx context.Context, tx *gorm.DB, deployment *models.Deployment) error {
	return nil // テストでは使用しない
}
